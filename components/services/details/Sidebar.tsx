...
export default function Sidebar() {
  ...
  return (
    <div className="flex justify-center items-center">
      <div className="w-[70%] rounded-2xl bg-[#F4F4F4] h-[30rem] py-4 px-4 grid gap-3">
        <h3 className="text-[25px]">Service List</h3>

        {serviceCards.map((service, index) => {
          const isActive =
            activeStatus?.toString().toLowerCase() ===
            ...
            <div key={index} className="flex items-center gap-5">
              <Link href={`/services/${service.id.toLowerCase()}`}>
                <button
                  ...
                  } px-4 py-2 rounded-md flex gap-6 w-[18rem] justify-between cursor-pointer`}
                >
                  {service.title}
